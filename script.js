(() => {
  ...
  const syncNavState = () => {
    ...
    primaryNav.hidden = menuToggle.getAttribute("aria-expanded") !== "true";
  };
  ...
  const closeNav = () => {
    if (!primaryNav || !menuToggle) return;
    menuToggle.setAttribute("aria-expanded", "false");
    if (mobileNavQuery.matches) primaryNav.hidden = true;
  };
  ...
  if (copyButton && eventCopySource) {
    copyButton.addEventListener("click", async () => {
      ...
      const copyText = [heading ? heading.innerText : "", eventCopySource.innerText].filter(Boolean).join("\n\n").trim();
      try {
  ...
  const closeGallery = () => {
    ...
    if (galleryModalPlaceholder) galleryModalPlaceholder.hidden = false;
  ...
  const openGallery = (button) => {
    ...
    modalTrigger = button;
    galleryModalTitle.textContent = button.dataset.galleryTitle || "Gallery image";
    galleryModalCaption.textContent = button.dataset.galleryCaption || "";
    galleryModalImage.alt = button.dataset.galleryAlt || "Gallery image";
    galleryModalImage.classList.remove("is-loaded");
    if (galleryModalPlaceholder) galleryModalPlaceholder.hidden = false;
    const src = button.dataset.gallerySrc || "";
    if (src) galleryModalImage.src = src;
  ...
  galleryButtons.forEach((button) => button.addEventListener("click", () => openGallery(button)));
  galleryCloseButtons.forEach((button) => button.addEventListener("click", closeGallery));

  if (galleryModal) {
    galleryModal.addEventListener("click", (event) => {
      if (event.target.matches("[data-gallery-close]")) closeGallery();
    });
  }

  window.addEventListener("keydown", (event) => {
    if (event.key === "Escape") {
      if (galleryModal && !galleryModal.hidden) return closeGallery();
      if (primaryNav && !primaryNav.hidden && mobileNavQuery.matches) closeNav();
    }
  });
  ...
  if (galleryModalImage) {
    galleryModalImage.addEventListener("load", () => {
      ...
      if (galleryModalPlaceholder) galleryModalPlaceholder.hidden = true;
    });
